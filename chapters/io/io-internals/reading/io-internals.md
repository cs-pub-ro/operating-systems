...
[Quiz](../drills/questions/strace-printf.md)
...
[Quiz](..//drills/questions/flush-libc-buffer.md)
...
Navigate to `buffering/support/no_buffering.c`.
...
1. Navigate to `buffering/support/diy_buffering.c`.
...
In the [following section](../../zero-copy/reading/zero-copy.md), we will see how to make use of this internal buffering to optimise network requests.

Notice that the script in `buffering/support/benchmark_buffering.sh` also uses `echo 3 > /proc/sys/vm/drop_caches`.
That section [in the Arena](../../arena/reading/arena.md#to-drop-or-not-to-drop) that we mentioned earlier is becoming even more interesting.
