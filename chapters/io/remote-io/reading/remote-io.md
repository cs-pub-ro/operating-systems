...
Remember [file handlers](../../file-handlers/reading/file-handlers.md)?
You should.
...
Navigate to `send-receive/support/` and take a look at both `sender.py` and `receiver.py`.
...
We'll explain the arguments in the [next section](../../networking-101/reading/networking-101.md#udp).
One thing to note here is that **sockets are file descriptors too.**

The server displays its PID.
Give it as an argument to `lsof`, like you did in [the section on Redirections](../../redirections/reading/redirections.md), to visualise the file descriptors opened by `receiver.py`.

[Quiz 1](../drills/questions/receiver-socket-fd.md)
...
[Quiz 2](../drills/questions/bind-error-cause.md)
...
1. Use the C sockets API to replicate the behavior of `sender.py` and `receiver.py`.
    ...
    Start from the skeleton in `send-receive/support`.
...
1. Use the API you've just learned about to fill in the `TODO`s in `receive-challenges/support/receive_net_dgram_socket.c`.
