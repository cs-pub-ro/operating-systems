...
def main():
    parser = argparse.ArgumentParser(description='Generate skeletons sources from reference solution sources')
    parser.add_argument('--input', help='input directory to process files', required=True)
    parser.add_argument('--output', help='output directory to copy processed files', required=True)
    ...
    for root, dirs, files in os.walk(args.input):
        ...
        for src in files:
            if re.match("Makefile.*$", src) \
                ...
                replace_pairs = [ ("; ", "") ]
                end_string = None
            elif re.match(".*\.[ch]$", src) or re.match(".*\.cpp$", src) \
                    or re.match(".*\.hpp$", src) or re.match(".*\.go$", src):
                pattern = "(.*/\*\s*TODO)([ 0-9]*)(:.*)"
