...
As a side note, the critical sections in `labs/lab-08/tasks/race-condition/support/c/race_condition_mutex.c` are also atomic once they are wrapped between calls to `pthread_mutex_lock()` and `pthread_mutex_unlock()`.
...
Go to `labs/lab-08/tasks/race-condition-atomic/` and run:

```bash
make skels
```

Now enter `labs/lab-08/tasks/race-condition-atomic/support/src/race_condition_atomic.c` and complete the function `decrement_var()`.
...
Go to `labs/lab-08/tasks/race-condition-atomic/support/race_condition_atomic2.c`, compile and run the code.
